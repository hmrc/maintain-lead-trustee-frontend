...
class LeadTrusteeIndividualPrintHelperSpec extends SpecBase {
  ...
  private val passportOrIdCard: CombinedPassportOrIdCard =
    CombinedPassportOrIdCard("DE", "1234567890", LocalDate.of(1996, 2, 3))

  val baseAnswers: UserAnswers = emptyUserAnswers
    .set(NamePage, name)
    .success
    .value
    .set(DateOfBirthPage, LocalDate.of(1996, 2, 3))
    .success
    .value
    ...
    "generate lead trustee organisation section" when {
      ...
      "not fully matched" in {
        ...
        result mustBe AnswerSection(
          ...
          rows = Seq(
            AnswerRow(
              label = messages("leadtrustee.individual.name.checkYourAnswersLabel"),
              answer = Html("Lead Trustee"),
              changeUrl = Some(NameController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages("leadtrustee.individual.dateOfBirth.checkYourAnswersLabel", name.displayName),
              answer = Html("3 February 1996"),
              changeUrl = Some(DateOfBirthController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages(
                "leadtrustee.individual.countryOfNationalityInTheUkYesNo.checkYourAnswersLabel",
                name.displayName
              ),
              answer = Html("No"),
              changeUrl = Some(CountryOfNationalityInTheUkYesNoController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages("leadtrustee.individual.countryOfNationality.checkYourAnswersLabel", name.displayName),
              answer = Html("Germany"),
              changeUrl = Some(CountryOfNationalityController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages("leadtrustee.individual.ukCitizen.checkYourAnswersLabel", name.displayName),
              answer = Html("Yes"),
              changeUrl = Some(UkCitizenController.onPageLoad().url)
            ),
            AnswerRow(
              label =
                messages("leadtrustee.individual.nationalInsuranceNumber.checkYourAnswersLabel", name.displayName),
              answer = Html("AA 00 00 00 A"),
              changeUrl = Some(NationalInsuranceNumberController.onPageLoad().url)
            ),
            AnswerRow(
              label =
                messages("leadtrustee.individual.passportOrIdCardDetails.checkYourAnswersLabel", name.displayName),
              answer = Html("Germany<br />Number ending 7890<br />3 February 1996"),
              changeUrl = None,
              canEdit = false
            ),
            AnswerRow(
              label = messages(
                "leadtrustee.individual.countryOfResidenceInTheUkYesNo.checkYourAnswersLabel",
                name.displayName
              ),
              answer = Html("No"),
            ...
            AnswerRow(
              label = messages("leadtrustee.individual.countryOfResidence.checkYourAnswersLabel", name.displayName),
              answer = Html("Germany"),
              changeUrl = Some(CountryOfResidenceController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages("leadtrustee.individual.liveInTheUkYesNo.checkYourAnswersLabel", name.displayName),
              answer = Html("Yes"),
              changeUrl = Some(LiveInTheUkYesNoController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages("leadtrustee.individual.ukAddress.checkYourAnswersLabel", name.displayName),
              answer = Html("value 1<br />value 2<br />AB1 1AB"),
              changeUrl = Some(UkAddressController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages("leadtrustee.individual.nonUkAddress.checkYourAnswersLabel", name.displayName),
              answer = Html("value 1<br />value 2<br />Germany"),
              changeUrl = Some(NonUkAddressController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages("leadtrustee.individual.emailAddressYesNo.checkYourAnswersLabel", name.displayName),
              answer = Html("Yes"),
              changeUrl = Some(EmailAddressYesNoController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages("leadtrustee.individual.emailAddress.checkYourAnswersLabel", name.displayName),
              answer = Html("email"),
              changeUrl = Some(EmailAddressController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages("leadtrustee.individual.telephoneNumber.checkYourAnswersLabel", name.displayName),
              answer = Html("tel"),
      ...
      "must generate a change link for combined passport ID question if it is not known to ETMP" in {
        ...
        val userAnswers = baseAnswers
          ...
          .success
          .value
          .set(PassportOrIdCardDetailsPage, passportOrIdCard.copy(detailsType = CombinedProvisional))
          .success
          .value
        ...
        result mustBe AnswerSection(
          ...
          rows = Seq(
            ...
            AnswerRow(
              label =
                messages("leadtrustee.individual.passportOrIdCardDetails.checkYourAnswersLabel", name.displayName),
              answer = Html("Germany<br />1234567890<br />3 February 1996"),
              changeUrl = Some(PassportOrIdCardController.onPageLoad().url)
            ),
            AnswerRow(
              label = messages(
                "leadtrustee.individual.countryOfResidenceInTheUkYesNo.checkYourAnswersLabel",
      ...
      "must not generate a change link for any combined passport/id question that is known to ETMP" in {
        ...
        val userAnswers = baseAnswers
          ...
          .success
          .value
          .set(PassportOrIdCardDetailsPage, passportOrIdCard.copy(detailsType = Combined))
          .success
          .value
