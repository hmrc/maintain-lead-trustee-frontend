...
class PassportOrIdCardController @Inject() (
  ...
  private val form: Form[CombinedPassportOrIdCard] =
    formProvider.withPrefix("leadtrustee.individual.passportOrIdCardDetails")
          ...
          for {
            ...
            detailsType     = {
              maybeOldAnswer match {
                case Some(oldAnswer) if oldAnswer.number == newAnswer.number && !oldAnswer.detailsType.isProvisional =>
                  Combined
            ...
            updatedAnswers <-
              Future.fromTry(
                request.userAnswers.set(PassportOrIdCardDetailsPage, newAnswer.copy(detailsType = detailsType))
              )
